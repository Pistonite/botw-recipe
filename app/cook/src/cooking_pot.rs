...
use crate::recipe::{self, RecipeData, Recipes};
use crate::ingr::{self, Ingredient, Ingredients};
use crate::tag::Tag;
use crate::{debugln, CookResult, Error};
...
impl CookingPot {
    ...
    pub fn cook_by_name<A: IntoIterator<Item=T>, T: AsRef<str>>(&self, names: A) -> Result<CookResult, Error> {
    ...
    pub fn cook<A: IntoIterator<Item=T>, T: Into<Actor>>(&self, actors: A) -> Result<CookResult, Error> {
        let ingrs = actors
            .into_iter()
            .filter_map(|x|{ 
                let x = x.into();
        ...
        if effect == CookEffect::LifeMaxUp {
            reference!("hearty effect", uking::CookingMgr::cookAdjustItem(), life_recover);
        ...
        Ok(CookResult { item: recipe.item(), data: output, crit_rng_hp })
    }
    ...
    fn calc_ingredient_boost(&self, ingrs: &[&Ingredient], recipe: &RecipeData, output: &mut CookData) -> Result<(CookEffect, bool), Error> {
        ...
        reference!("every ingredient (including duplicated ones) count", uking::CookingMgr::cookCalcIngredientsBoost());
        for ingr in ingrs {
            reference!("enemy boosts added early", uking::CookingMgr::cookCalcIngredientsBoost(), tags::CookEnemy);
            if ingr.tags.contains(&Tag::CookEnemy) {
                ...
                reference!("potency only added for that effect", uking::CookingMgr::cookCalcIngredientsBoost(), cureItemEffectType);
                if is_effect_item {
                    potency += ingr.effect_level;
                }
            }
        }
        debugln!("Base effect duration is {}*30 + (effect item)*{} + enemy boost = {}", ingrs.len(),  base_time, time);

        output.effect_id = effect.game_repr_f32();

        if has_multiple_effect {
            reference!("clears effect if multiple effects", uking::CookingMgr::cookCalcIngredientsBoost(), effect_found);
        ...
        if output.effect_id != -1.0 {
            if output.effect_level > effect.data().max_level as f32 {
                ...
                return Err(Error::Data(format!("Effect level {} is too high for effect {:?}", output.effect_level, effect)));
            }
        }
    ...
    fn calc_crit_boost(unique_ingrs: &[&Ingredient], effect: CookEffect, output: &mut CookData) -> bool {
        let mut crit_chance = 
        unique_ingrs.iter()
