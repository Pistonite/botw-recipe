...
import { Button, Caption1, createTableColumn, DataGrid, DataGridBody, DataGridCell, DataGridCellFocusMode, DataGridHeader, DataGridHeaderCell, DataGridRow, Label, LabelProps, makeStyles, shorthands, TableCellLayout, TableColumnId, ToggleButton } from "@fluentui/react-components";
import { Add20Filled, Delete20Regular, Star20Filled, Star20Regular } from "@fluentui/react-icons";
...
const ItemActorSelectionColumns = [
    createTableColumn<ItemActorSelectionRowProps>({
        columnId: "actor",
        renderHeaderCell: (t) => (t as (key: string)=>string)("filter.selection.actor"),
    ...
    createTableColumn<ItemActorSelectionRowProps>({
        columnId: "option",
        renderHeaderCell: (t) => (t as (key: string)=>string)("filter.selection.option"),
        renderCell: ({
            actor, included, 
            favorited,
            onSelectInclude, onSelectFavorite, t
        }) => {
            ...
            return <>
                <ToggleButton 
                    ...
                    icon={favoriteSelected ? <Star20Filled /> : <Star20Regular />}
                />
                <Button
                    aria-label={included ? t("filter.selection.option.exclude") : t("filter.selection.option.include")}
...
export const ItemActorSelection: React.FC<ItemActorSelectionProps> = ({ 
    ...
    favorited, 
    searchText, 
    showExcluded, actorSubtitles, setIncluded, setFavorited }) => {
    const search = useItemSearch();
    const searchResult = useMemo(() => search(searchText), [searchText, search]);
    const actors = useMemo(() => {
        let baseActors = showExcluded ? getActors() : included;
        if (searchResult) {
    ...
    const rowProps = useMemo(() => {
        ...
        const onSelectInclude = (actor: Actor, include: boolean) => {
            setIncluded(include ? [...included, actor] : included.filter(a => a !== actor));
        };
        const onSelectFavorite = (actor: Actor, isFavorited: boolean) => {
            setFavorited(isFavorited ? [...favorited, actor] : favorited.filter(a => a !== actor));
        };
    ...
    }, [actors, included, favorited, actorSubtitles, setIncluded, setFavorited, t]);
...
export const ItemActorLabel: React.FC<ItemActorProps & LabelProps> = ({ actor, ...rest }) => {
    const {t} = useTranslation();
    return (
        <Label {...rest}>{t(`actor.${ActorToName[actor]}`)}</Label>
    );
};
...
export const ItemActorImg: React.FC<ItemActorProps> = memo(({actor}) => {
    ...
    return (
        <div className={styles.iconContainer} aria-hidden>
                <img className={styles.icon} src={`/actors/${ActorToName[actor]}.png`} />
        </div>
    );
});
