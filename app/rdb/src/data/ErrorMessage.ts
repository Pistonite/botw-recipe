...
export const getErrorMessage = (error: HostError): string => {
    ...
    switch (error.type) {
        case "IOError": return getIOErrorMessage(error.data);
        case "PoisonError": return i18n.t("error.message.poison", { message: error.data });
        case "ExecutorUnavailable": return i18n.t("error.message.executor.unavailable");
        case "DatabaseError": return getDatabaseErrorMessage(error.data);
        case "Generic": return i18n.t("error.generic", { message: error.data });
        default:
...
const getDatabaseErrorMessage = (error: DatabaseError): string => {
    const type = error.type;
    switch (error.type) {
        case "IO": return getIOErrorMessage(error.data);
        case "YAML": return getYAMLErrorMessage(error.data);
        case "Cooking": return getCookingErrorMessage(error.data);
        case "Locked": return i18n.t("error.message.database.locked");
        case "MissingIndex": 
        case "InvalidIndexChunkCount":
        case "MissingChunk":
        case "InvalidChunkSize":
            return getInvalidDatabaseErrorMessage(type);
        case "InvalidRecipeId": return getInvalidRecipeIdErrorMessage(error.data);
        default:
