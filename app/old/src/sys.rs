...
pub fn to_usize(input: u64) -> usize {
    input
        .try_into()
        .unwrap_or_else(|_| panic!("Cannot convert {} to usize", input))
}

/// Create temp directory
pub fn create_temp_dir() -> Result<(), String> {
    fs::create_dir_all("temp")
        .map_err(|err| format!("Cannot create temp directory: {}", err.to_string()))
}
...
pub fn open_random_access_file(path: &str) -> Result<RandomAccessFile, String> {
    RandomAccessFile::open(&path).map_err(|err| {
        format!(
            "Cannot open {} for random access: {}",
            &path,
            err.to_string()
        )
    })
...
pub fn read_random_access_file(
    path: &str,
    ...
    file.read_at(offset, buf)
        .map_err(|err| format!("error accessing {}: {}", &path, err.to_string()))
}
...
fn open_file_r(path: &str) -> Result<File, String> {
    File::open(&path)
        .map_err(|err| format!("Cannot open {} for reading: {}", &path, err.to_string()))
}

pub fn open_file_wbuf(path: &str) -> Result<BufWriter<File>, String> {
    let output_file = File::create(&path)
        .map_err(|err| format!("Cannot open file for writing: {}", err.to_string()))?;
    Ok(BufWriter::new(output_file))
}

pub fn read_file<F>(path: &str, reader: &mut dyn Read, mut op: F) -> Result<(), String>
where
    F: FnMut(usize, &[u8]) -> Result<(), String>,
{
    let mut buffer = [0u8; FILE_BUF_SIZE];
    loop {
        let bytes_read = reader
            .read(&mut buffer)
            .map_err(|err| format!("error reading {}: {}", path, err.to_string()))?;
...
pub fn write_file(path: &str, writer: &mut dyn Write, buf: &[u8]) -> Result<usize, String> {
    writer
        .write(buf)
        .map_err(|err| format!("error writing {}: {}", &path, err.to_string()))
}
...
pub fn split_input(input: &str, thread_count: usize) -> Result<u64, String> {
    ...
    let size = input_file
        .metadata()
        .map_err(|err| format!("Cannot read size of {}: {}", &input_path, err.to_string()))?
        .len();
        ...
        |bytes_read: usize, buf: &[u8]| {
            ...
            if current_size >= chunk_size {
                ...
                if current_thread_id >= thread_count {
                    panic!(
                        "Current thread id {} exceed max thread count {}",
                        current_thread_id, thread_count
                    );
                }
