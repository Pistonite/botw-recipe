...
use super::{CHUNK_SIZE, NUM_RECORD, RECORD_SIZE};
const THREADS: usize = 32;

fn multi_process<S, R, H>(
    ...
    S: Fn(usize, Arc<AtomicU64>, Arc<AtomicU64>) -> JoinHandle<R>,
...
pub fn run_sample(
    ...
    multi_process(
        ...
        false,
        total_recipe_count,
        |chunk_id, processed, _| run_sample_for_chunk(chunk_id, running.clone(), processed.clone()),
        |result| {
            result.map(|result| {
    ...
    for i in 0..NUM_ITEMS {
        ...
        if count > 0 {
            ...
            println!(
                "{} in {}/{} recipes ({})",
                &material, count, total_recipe_count, percentage
            );
        }
    }
        ...
        loop {
            ...
            match found {
                Some(ref found) => {
                    ...
                    print!(
                        "\r--> Progress: {}/{} ({}). Found {} Recipes   ",
                        processed_value, total, percentage, found_value
                    );
                }
                None => {
                    print!(
                        "\r--> Progress: {}/{} ({}).   ",
                        processed_value, total, percentage
                    );
                }
            }
        ...
        sys::read_file(
            ...
            |bytes_read, buf| {
                ...
                let out_bytes: Vec<u8> =
                    out_buffer.iter().flat_map(|id| id.to_be_bytes()).collect();
...
fn run_sample_for_chunk(
    chunk_id: usize,
    running: Arc<AtomicBool>,
    processed: Arc<AtomicU64>,
) -> JoinHandle<Result<[u64; recipe::NUM_ITEMS], String>> {
    thread::spawn(move || {
